<?php
	class StudentTest{
		...
		public function print_test($user_type){
			...
			$student_answer_statement = $this->db->prepare("SELECT answer_given, points_received FROM student_test_answers WHERE student_id = ? AND test_id = ? AND question_id = ?");
			...
			if($question_statement->num_rows > 0){
				...
				while($question_statement->fetch()){					
					...
					$student_answer_statement->bind_result($answer_given, $points_received);
					$student_answer_statement->fetch();
					
					// Print the question
					$this->print_question($question_id, $question_text, $user_type, $question_type, $question_weight, $points_received);
		...
		public function print_question($question_id, $question_text, $user_type, $question_type, $question_weight, $points_received){
		...
		public function print_answer($is_correct, $answer_content, $question_type, $user_type, $question_id, $answer_id, $answer_given){
			...
			case self::TRUE_FALSE_QUESTION_TYPE:
				...
				if($answer_content == 'T')
					if($answer_given == $answer_content || $answer_given == null){
